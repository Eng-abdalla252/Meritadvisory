    ...
    try {
        ...
        if (fs.existsSync(submissionsPath)) {
            try {
                ...
                console.error(
                    "Error parsing questionnaire submissions:",
                    e
                )
            }
        }
        ...
        if (fs.existsSync(leadsPath)) {
            try {
                const leadsContent = fs.readFileSync(
                    leadsPath,
                    'utf8'
                )
        ...
        if (resend) {
            ...
            await resend.emails.send({
                ...
                subject:
                    `New Project Questionnaire from ${data.companyName} - ${data.blueprintData?.name || 'No Blueprint'}`,

                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">

                        <h2 style="color: #b22222; border-bottom: 1px solid #eee; padding-bottom: 10px;">
        ...
        if (result.success) {
            return NextResponse.json({
                message:
                    "Questionnaire submitted and synced to Odoo CRM",
            ...
            return NextResponse.json({
                message:
                    "Questionnaire submitted successfully (Odoo sync pending)",
